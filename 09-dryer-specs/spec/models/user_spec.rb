...
RSpec.describe User, type: :model do
  ...
  describe "API token generation" do
    it "sets a new user's API token" do
  ...
  it "sets an avatar" do
    user = FactoryBot.build(:user, nickname: "test_user")
    mock_io = StringIO.new("fake image data")
    allow(URI).to receive(:open).with("https://api.dicebear.com/8.x/thumbs/png?seed=test_user").and_return(mock_io)
    allow(user.avatar).to receive(:attach).and_call_original
    user.save

    expect(URI).to have_received(:open).with("https://api.dicebear.com/8.x/thumbs/png?seed=test_user")
